...
async transcribeMediaSource(source: string, applyVad: boolean, postProcess: boolean) : Promise<Result<MediaTranscriptionResult, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("transcribe_media_source", { source, applyVad, postProcess }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type EngineType = "Whisper" | "Parakeet" | "Moonshine" | "MoonshineStreaming" | "SenseVoice" | "GigaAM" | "Canary" | "Cohere" | "FunASR"
...
export type ModelFile = { relative_path: string; url: string; sha256: string | null }
export type ModelInfo = { id: string; name: string; description: string; filename: string; url: string | null; sha256: string | null; size_mb: number; is_downloaded: boolean; is_downloading: boolean; partial_size: number; is_directory: boolean; engine_type: EngineType; accuracy_score: number; speed_score: number; supports_translation: boolean; is_recommended: boolean; supported_languages: string[]; supports_language_selection: boolean; is_custom: boolean; files: ModelFile[] }
export type ModelLoadStatus = { is_loaded: boolean; current_model: string | null }
export type MediaTranscriptionResult = { text: string }
